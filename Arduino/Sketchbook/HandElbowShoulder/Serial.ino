...
{  
  ...
  if (Serial.available() > 0)
  {
    ...
    switch (channel)
    {
    case 1:  // Shoulder Pan via H-Bridge
      ShoulderPanSetpoint = myMap((double)cmdValue, 800, 2200, -90, 90);
      break;
    case 2:  // Shoulder Tilt via H-Bridge
      ShoulderTiltSetpoint = myMap((double)cmdValue, 800, 2200, -90, 90);
      break;
    case 3:  // Shoulder Turn via H-Bridge
      ShoulderTurnSetpoint = myMap((double)cmdValue, 800, 2200, -90, 90);
      break;
    case 4:  // Elbow via H-Bridge
      ElbowSetpoint = myMap((double)cmdValue, 800, 2200, -90, 90);
      break;
    }
    sei();
    ...
    switch (channel)
    {
      case 5: // thumb
      case 6: // index finger
      case 7: // middle finger
      case 8: // pinky finger
      case 9: // wrist angle
        sendI2cPacket(channel, cmd, cmdValue, checksum);
        break;
    }
...
void  ReportState()
{
  ...
  //Serial.print(" ");
  Serial.print(_ElbowSetpoint);
  Serial.print(" ");
  Serial.print(_ElbowMeasured);
  Serial.print(" ");
  Serial.println(_ElbowMotorChannelPower);
}
...
//          servos[g_channel].writeMicroseconds(g_value);
          ...
          switch (g_channel)
          {
          case 3:  // Elbow via H-Bridge
            ElbowSetpoint = myMap((double)g_value, 800, 2200, -90, 90);
            break;
          case 5:  // Shoulder Turn via H-Bridge
            ShoulderTurnSetpoint = myMap((double)g_value, 800, 2200, -90, 90);
            break;
