...
boolean Angle_calculate(void)
{
  ...
  double angle_dot_tmp = GyroY / 14.375;       // the angular velocity
...
void Kalman_Filter(double angle_m, double gyro_m)   
{
  ...
  P[1][0] -= K_1 * t_0;
  P[1][1] -= K_1 * t_1;
  angle += K_0 * angle_err;  // Optimal (estimated) angle
  q_bias += K_1 * angle_err;
  angle_dot = gyro_m - q_bias; // Optimal (estimated) angular velocity
}
